...
(1) First step: open the Terminal.

If you have an Apple Silicon Mac, download and install the command-line `conda`:
...
On an older Intel Mac, the commands are:
...
(2) Once the installation is done, you can delete the installer -- run one of the two lines below:

```sh
/bin/rm Miniconda3-latest-MacOSX-x86_64.sh
/bin/rm Anaconda3-2025.12-2-MacOSX-arm64.sh
```

(3) Now, restart the Terminal, and try typing `conda` -- it should be able to find the command.

(4) Next, create a new virtual environment (you might need to downgrade your Python to 3.13 on Intel Macs):

```sh
conda create --name env-conda-dhsi python=3.14
```

(5) Then activate the environment and install the packages:
...
To install `polars` package, use one of these lines:
...
(6) To start JupyterLab, type `jupyter lab`.

(7) Finally, to deactivate the environment:
