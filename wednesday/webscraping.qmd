...
The HTML head section of the parsed data can be accessed with `mainpage.head` (it is a bit long, so let's not print it).
...
But things are a bit more complicated in this case than it sometimes is. Let's try things on the first link:
...
Unfortunately, this is not a full URL and you need to prepend "https://www.theoi.com/greek-mythology/" to it:

```{python}
subcat_url_test = "https://www.theoi.com/greek-mythology/" + mainpage_captions[
    0
].get("href")
```

Let's see whether we got the first URL:
...
First, we create an empty list that we will fill with the URLs as the loops runs:
...
for caption in mainpage_captions:
    subcats_urls.append("https://www.theoi.com/greek-mythology/" + caption.get("href"))
```
...
The information is here, but it is not formatted in the same fashion as the links we had in the previous part, so there will be a bit more work to recreate the URL of the gods pages. Luckily, it is the same operation for all links so we will be able to do this with a for loop.
...
Now, to recreate the actual URL, we first need to remove `../Olympios/`:

```{python}
subcat0_captions[0].get("href").replace("../Olympios/", "")
```

Then you prepend `https://www.theoi.com/Olympios/`:

```{python}
"https://www.theoi.com/Olympios/" + subcat0_captions[0].get("href").replace("../Olympios/", "")
```

Now that we checked that the code works on a single entry, we can do it on all elements of the list:

```{python}
gods_urls0 = []

for caption in subcat0_captions:
    gods_urls0.append("https://www.theoi.com/Olympios/" + caption.get("href").replace("../Olympios/", ""))
```
...
for subcats_url in subcats_urls:
    ...
    for caption in subcat_captions:
        gods_urls.append("https://www.theoi.com/Olympios/" + caption.get("href").replace("../Olympios/", ""))
...
Let's check the length of our filled in list:

```{python}
len(gods_urls)
```

There are 174 Greek gods in the Theoi project.

As another sanity check, let's check the last 5:

```{python}
gods_urls[-5:]
```

There is a good news and a bad news... The good news is that we have the right gods. The bad news is that the URLs are different for each subcategory of gods. This shows you the importance of doing many sanity checks.

We need to fix our code.

Instead of removing `../Olympios/` as we did previously (which is a form of hard-coding), we will use [regular expressions](https://en.wikipedia.org/wiki/Regular_expression) to make our code more broadly applicable.

First we need to import the regular expression module from the standard library:

```{python}
import re
```

Then we need to replace the content of our nested for loops to remove `../` from all the extracted gods URLs and we then need to prepend a shorter URL (`https://www.theoi.com/`) so that the subcategory of the god in the URL is replaced by the proper one:
...
for subcats_url in subcats_urls:
    ...
    for caption in subcat_captions:
        ...
        href = re.sub(r"\.\./", "", href)
        gods_urls.append("https://www.theoi.com/" + href)
    # Add a delay at each iteration
    time.sleep(0.1)
```
...
Remember that our goal was to get the gods names and their parents names. As always, we test the code on a single element (here one god URL). Let's extract the first gods URL from the list:
...
We then use a regular expression again to clean the part of the title that we are not interested in:
...
```{python}
god0_parents = god0.select("tr:nth-child(1) td")
god0_parents
```

It worked, but our element is in a list like object (as you can see it is in square brackets), so we need to index it out of that objects:

```{python}
god0_parents_item = god0_parents[0]
god0_parents_item
```

Now our element is in a `td` tag so we extract it with the `get_text` method:

```{python}
god0_parents_item.get_text()
```

🥳

We now need to create a loop to do this for all gods:
...
for god_url in gods_urls:
    ...
    god_name = re.sub(r"\s*-.*$", "", title)
    name = god_name.capitalize()
    god_parents = god.select("tr:nth-child(1) td")
    parents = god_parents[0].get_text()
    # Save the god's name and its parents' names in a tuple
    gods_info.append((name, parents))
```

We are getting an error message because the site has inconsistencies so the code works for most, but not all gods' pages (and the error message is triggered when the loop runs for this (or these) page(s).

Let's add an if statement that will print which god(s) are creating problem for us:
...
for god_url in gods_urls:
    ...
    god_parents = god.select("tr:nth-child(1) td")
    if not god_parents:
        print(f"No parents found for {god_url}")
        continue
    parents = god_parents[0].get_text()
    gods_info.append((name, parents))
```

As you can see, a single god is causing this index error: Soteria. That's because it does not have parents. Let's ignore that god.

But we have another error because one of the requests returns a 404 due to a problem in the website. Let's ignore this for now.

We now have a list of tuples with our result. Here are the first 5 tuples:
...
A DataFrame would be a lot more convenient than a list of tuples to hold our result. Let's create one with Polars:
...
*Because parquet is so much more efficient and the right format to use for large datasets, I like using it all the time everywhere.*
