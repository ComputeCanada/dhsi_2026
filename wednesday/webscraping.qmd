...
This returns a BeautifulSoup `ResultSet` object which behaves like a list.
...
This URL might look peculiar to you but it is actually totally valid: `..` represents the parent directory and `https://www.theoi.com/Olympios/../Olympios/Aphrodite.html` is actually exactly the same as `https://www.theoi.com/Olympios/Aphrodite.html`.
...
There appears to be 174 Greek gods in the Theoi project.
...
We can now create a loop to run this code on all the elements of the result set:
...
for god_url in gods_urls:
    ...
    if r.status_code != 200:
        print(f"{god_url} has a status code of {r.status_code}")
    else:
        god = BeautifulSoup(r.text, "html.parser")
        title = god.title.get_text()
        god_name = re.sub(r"\s*-.*$", "", title)
        ...
        if len(god_parents) == 0:
            print(f"{god_url} does not have data about parents")
        else:
            parents = god_parents[0].get_text()
            gods_info.append((name, parents))
```

As you can see, one URL returns a 404 and a number of gods do not have data about their parents in the format we are scraping. We are excluding them from the results and we now have a list of tuples. Here are the first 5 tuples:
